...
describe("public discovery", () => {
  ...
  test("publishes only explicit public metadata on the configured site origin", async () => {
  ...
  test("CLI generates and retrieves profiles without a login; never overwrites files", async () => {
    ...
    expect((await run("discovery", "create", "--origin", localOrigin, "--api-base", localOrigin, "--out", localPath, "--allow-local")).exit).toBe(0);
