...
export async function createApp(baseStore: Store): Promise<App> {
...
export async function startServer(opts: { store: Store; port?: number; hostname?: string }): Promise<RunningServer> {
  const app = await createApp(opts.store);
