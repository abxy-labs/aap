...
export async function directory(store: Store): Promise<{ object: "list"; data: { object: "directory_entry"; hash: string }[]; has_more: false; url: string }> {
  const latest = new Map<string, { version: number; tier: string }>();
  for (const p of await store.listPolicies()) {
    const cur = latest.get(p.origin);
    if (!cur || p.version > cur.version) latest.set(p.origin, { version: p.version, tier: p.tier });
  }
  const data = [...latest.entries()].filter(([, v]) => v.tier !== "none").map(([origin]) => ({ object: "directory_entry" as const, hash: hashOrigin(origin) })).sort((a, b) => a.hash.localeCompare(b.hash));
