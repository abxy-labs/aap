...
Resources (create, retrieve, list, and the verbs shown)
  ...
  directory       list
  discovery       create --origin URL --api-base URL [--credentials] [--out FILE] [--allow-local] (offline)
...
async function main(argv: string[]): Promise<number> {
  ...
  switch (cmd) {
    ...
    case "discovery": {
      ...
      if (sub !== "create") throw new UsageError("usage: discovery create --origin URL --api-base URL [--credentials] [--out FILE] | discovery retrieve ORIGIN");
      const profile = createDiscoveryProfile({ origin: str(flags, "origin", true)!, apiBase: str(flags, "api-base", true)!, credentials: bool(flags, "credentials") }, opts);
